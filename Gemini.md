...
\- Skills are SOP-grade â€œcapability packagesâ€ stored as \*\*self-contained folders\*\*.  
...
\- Skills are \*\*discoverable on-demand\*\*: the router matches user intent to the Skillâ€™s metadata (especially the \`description\`) and loads the full Skill instructions only when needed.  
...
\- You are the glue between intent and execution:  
  \- You donâ€™t â€œwing itâ€ for repeatable workflows.  
...
\- \`description\` (\*\*mandatory\*\*; â€œtrigger phraseâ€ that must be specific)
...
4\. \*\*Constraints\*\* (â€œdo notâ€ rules, safety limits)  
...
Keep â€œheavyâ€ static text (legal templates, long references) in \`references/\` and instruct the agent to read it only when needed.
...
\- If it doesnâ€™t, create a script inside \`scripts/\` (or a shared script location) and then update the Skill.
...
\- \`.tmp/\` â€” All intermediate files (never commit; always regeneratable)  
\- \`.agent/skills/\` â€” Workspace-scoped Skills (project-specific)  
\- \`\~/.gemini/antigravity/skills/\` â€” Global Skills (reusable across projects)  
\- \`execution/\` â€” Shared deterministic scripts (optional; use when multiple Skills share tooling)  
\- \`.env\` â€” Environment variables and API keys  
\- \`credentials.json\`, \`token.json\` â€” OAuth credentials (gitignored)

\*\*Key principle:\*\* Local files are for processing. Deliverables should live in cloud services whenever possible.

\---

\#\# Migration Guide (Directives â†’ Skills)
...
\- Update wording:  
  \- â€œread the directiveâ€ â†’ â€œactivate the Skillâ€  
  \- â€œupdate the directiveâ€ â†’ â€œupdate the Skillâ€
